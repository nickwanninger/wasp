...
static int _vsnprintf(out_fct_type out, char *buffer, const size_t maxlen,
  ...
  while (*format) {
    ...
    switch (*format) {
      ...
      case 't':
        flags |= (sizeof(unsigned long long) == sizeof(long) ? FLAGS_LONG
                                                             : FLAGS_LONG_LONG);
        format++;
        break;
#endif
...
#if defined(PRINTF_SUPPORT_LONG_LONG)
        const bool is_ll = sizeof(unsigned long long) == sizeof(long long);
        if (is_ll) {
          idx = _ntoa_long_long(out, buffer, idx, maxlen,
                                (unsigned long long)va_arg(va, void *), false,
                                16U, precision, width, flags);
        } else {
#endif
          idx = _ntoa_long(
              out, buffer, idx, maxlen,
              (unsigned long)((unsigned long long)va_arg(va, void *)), false,
              16U, precision, width, flags);
#if defined(PRINTF_SUPPORT_LONG_LONG)
        }
#endif
...
int fctprintf(void (*out)(char character, void *arg), void *arg,
  ...
  const int ret =
      _vsnprintf(_out_fct, (char *)(unsigned long long)&out_fct_wrap,
                 (size_t)-1, format, va);
  va_end(va);
  return ret;
}
